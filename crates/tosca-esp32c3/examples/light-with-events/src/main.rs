...
async fn press_button(button: AnyPin<'static>, _notifier: Notifier<bool>) {
    // Input button
...
async fn change_led(led: AnyPin<'static>, _notifier: Notifier<bool>) {
    // Output led.
...
async fn main(spawner: Spawner) {
    ...
    let events_config = EventsConfig::new(
        spawner,
        stack,
        BrokerData::ip(IpAddress::v4(127, 0, 0, 1), 4000),
        device,
    );
    ...
    let device = EventsManager::config(events_config)
        ...
        .run_network_task()
        .await
        .expect("Error running the event manager");
