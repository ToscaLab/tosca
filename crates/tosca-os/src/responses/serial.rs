...
use super::{error::ErrorResponse, DeviceResponse, MandatoryResponse};
...
/// Creates a mandatory stateful [`DeviceResponse`] with a [`SerialResponse`].
#[inline]
    ...
    S: Clone + Send + Sync + 'static,
{
    move |route: Route, state: S| {
        MandatoryResponse::new(DeviceResponse::stateful(
            route,
            ResponseKind::Serial,
            handler,
            state,
...
/// Creates a stateful [`DeviceResponse`] with a [`SerialResponse`].
#[inline]
pub fn serial_stateful<H, T, S>(route: Route, handler: H) -> impl FnOnce(S) -> DeviceResponse
where
    H: Handler<T, S> + private::SerialTypeName<T>,
    T: 'static,
    S: Clone + Send + Sync + 'static,
{
    move |state: S| DeviceResponse::stateful(route, ResponseKind::Serial, handler, state)
}

/// Creates a mandatory stateless [`DeviceResponse`] with a [`SerialResponse`].
#[inline]
    ...
    S: Clone + Send + Sync + 'static,
{
    move |route: Route, _state: S| {
        MandatoryResponse::new(DeviceResponse::stateless(
            route,
            ResponseKind::Serial,
            handler,
        ))
...
/// Creates a stateless [`DeviceResponse`] with a [`SerialResponse`].
#[inline]
pub fn serial_stateless<H, T, S>(route: Route, handler: H) -> impl FnOnce(S) -> DeviceResponse
where
    H: Handler<T, ()> + private::SerialTypeName<T>,
    T: 'static,
    S: Clone + Send + Sync + 'static,
{
    move |_state: S| DeviceResponse::stateless(route, ResponseKind::Serial, handler)
}
